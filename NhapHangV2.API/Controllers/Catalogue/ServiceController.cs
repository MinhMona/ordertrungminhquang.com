...
namespace NhapHangV2.API.Controllers.Catalogue
{
    [Route("api/service")]
    [ApiController]
    [Description("Danh sách dịch vụ (trang chủ)")]
    public class ServiceController : ControllerBase
    {
        ...
        public ServiceController(IServiceProvider serviceProvider, ILogger<ServiceController> logger, IWebHostEnvironment env, IMapper mapper, IConfiguration configuration)
        {
            this.mapper = mapper;
            this.env = env;
        ...
        public async Task<AppDomainResult> GetById(int id)
        {
            ...
            var item = await serviceService.GetByIdAsync(id);
            if (item != null)
            {
        ...
        public async Task<AppDomainResult> AddItem([FromBody] ServiceRequest itemModel)
        {
            AppDomainResult appDomainResult = new AppDomainResult();
            ...
            if (ModelState.IsValid)
            {
                ...
                var item = mapper.Map<Entities.Catalogue.Service>(itemModel);
                ...
                if (item != null)
                {
                    ...
                    success = await serviceService.CreateAsync(item);
                    if (success)
                    {
        ...
        public async Task<AppDomainResult> UpdateItem([FromBody] ServiceRequest itemModel)
        {
            AppDomainResult appDomainResult = new AppDomainResult();
            ...
            if (ModelState.IsValid)
            {
                ...
                if (item != null)
                {
                    ...
                    //if (!string.IsNullOrEmpty(messageUserCheck))
                    //    throw new KeyNotFoundException(messageUserCheck);
                    success = await serviceService.UpdateAsync(item);
                    if (success)
                    {
                        appDomainResult.ResultCode = (int)HttpStatusCode.OK;
        ...
        public async Task<AppDomainResult> DeleteItem(int id)
        {
            ...
            bool success = await serviceService.DeleteAsync(id);
            if (success)
            {
                appDomainResult.ResultCode = (int)HttpStatusCode.OK;
                appDomainResult.Success = success;
        ...
        public async Task<AppDomainResult> Get([FromQuery] CatalogueSearch baseSearch)
        {
            ...
            if (ModelState.IsValid)
            {
                PagedList<Entities.Catalogue.Service> pagedData = await serviceService.GetPagedListData(baseSearch);
                PagedList<ServiceModel> pagedDataModel = mapper.Map<PagedList<ServiceModel>>(pagedData);
                appDomainResult = new AppDomainResult
                {
                    Data = pagedDataModel,
                    Success = true,
